    ...
    try:
        ...
        for i, page in enumerate(doc):
            ...
            if not page_text:
                ...
                if images:
                    logger.debug(f"Page {i+1} has {len(images)} images but no text.")
        ...
        logger.info(f"Total extracted: {extracted_len} characters")
        
        if extracted_len == 0:
            logger.warning("Extracted text is empty. PDF might be an image/scan.")
